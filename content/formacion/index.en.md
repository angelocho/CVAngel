---
title: "Education"
description: "My educational background and professional certifications"
date: 2026-03-04
params:
  author: "Ángel Bocalandro Ruiz"
---

# 🎓 Education

## 📜 Formal Studies

### Higher Vocational Training Cycle
**Technician in Network Computer System Administration**

- **Center**: I.E.S. Villablanca
- **Period**: 2020 - 2022
- **Duration**: 2 years
- **Description**: Comprehensive training in system administration, network management, cybersecurity, and network services

#### Acquired Knowledge:
- Windows and Linux system administration
- Network service management (DNS, DHCP, FTP, SSH, NFS, Samba)
- Network security (firewalls, VPNs, encryption)
- Database administration
- Virtualization
- Access and permission management (Active Directory)

#### 🏆 Final Course Project
**Kali Linux** - Implementation and use of offensive security tools

---

## 📚 Professional Certifications

### Cloud Certifications
- **AWS SysOps Administrator** - In progress
- **Microsoft Azure AZ-104** - Azure Administrator Associate
- **Microsoft Azure AZ-900** - Azure Fundamentals

### Training Platforms
- **Pluralsight** - Multiple courses on cloud infrastructure and DevOps tools
- **Linux Academy** - Kubernetes and containerization courses

---

## 🎯 Continuous Learning

I'm committed to continuous professional development through:
- Regular participation in DevOps and cloud infrastructure courses
- Hands-on experience with emerging technologies
- Professional communities and knowledge sharing
- Reading technical documentation and best practices
